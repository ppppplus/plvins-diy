...
class LineFeatureTracker:
	def __init__(self, extract_model, match_model, cams, max_cnt=150):
	...
	def readImage(self, new_img):
		...
		start_time = time()
		self.forwframe_['vecline'], self.forwframe_['descriptor']  = self.extractor.extractLines(self.new_frame)
		...
		if self.curframe_['vecline'].shape[0] > 0:
			...
			################### 将跟踪的线与没跟踪的线进行区分 #####################
			Vecline_new = np.zeros((3,0))
			Vecline_tracked = np.zeros((3,0))
			LineID_new = []
			LineID_tracked = []
			Descr_new = np.zeros((128,0))
			Descr_tracked = np.zeros((128,0))

			for i in range(lines_num):
				if self.forwframe_['lineID'][i] == -1 :	# -1表示当前ID对应的line没有track到
					self.forwframe_['lineID'][i] = self.allfeature_cnt	# 没有跟踪到的线则编号为新的
					self.allfeature_cnt = self.allfeature_cnt+1
					Vecline_new = np.append(Vecline_new, self.forwframe_['vecline'][i], axis=1)	# 取出没有跟踪到的线信息
					LineID_new.append(self.forwframe_['lineID'][i])
					Descr_new = np.append(Descr_new, self.forwframe_['descriptor'][i], axis=1)
				else:
					# 当前line已被track
					Vecline_tracked = np.append(Vecline_tracked, self.forwframe_['vecline'][i], axis=1)
					LineID_tracked.append(self.forwframe_['lineID'][i])
					Descr_tracked = np.append(Descr_tracked, self.forwframe_['descriptor'][i], axis=1)
