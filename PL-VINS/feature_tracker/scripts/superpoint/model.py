...
class SuperPointTrtFrontend(object):
  """ Wrapper around pytorch net to help with pre and post image processing. """
  def __init__(self, weights_path, nms_dist, conf_thresh, nn_thresh, engine_file_path,
               cuda=False):
    self.name = 'SuperPointTrt'
    self.cuda = cuda
    self.nms_dist = nms_dist
    self.conf_thresh = conf_thresh
    self.nn_thresh = nn_thresh # L2 descriptor distance for good match.
  ...
  def run(self, img):
    ...
    semi = trt_outputs[0].reshape((65,H//self.cell,W//self.cell))
    coarse_desc = trt_outputs[1].reshape((1,256,H//self.cell,W//self.cell))
...
class MyPointExtractModel:
  def __init__(self, params):
    ...
    if params["trt"]:
      print("=========trt mode=========")
      self.fe = SuperPointTrtFrontend(weights_path=self.params["weights_path"],
                            nms_dist=self.params["nms_dist"],
                            conf_thresh=self.params["conf_thresh"],
                            nn_thresh=self.params["nn_thresh"],
                            engine_file_path=self.params["engine_file_path"],
