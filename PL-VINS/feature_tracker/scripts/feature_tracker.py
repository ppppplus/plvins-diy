...
class FeatureTracker:
	def __init__(self, extract_model, track_model, cams, max_cnt=150):
	...
	def readImage(self, new_img):
		...
		start_time = time()
		self.forwframe_['keyPoint'], self.forwframe_['descriptor'], heatmap = self.extractor.extract_point(self.new_frame)
		...
		##################### 开始处理匹配的特征点 ###############################
		if self.curframe_['keyPoint'].shape[0] > 0:
			start_time = time()
