...
class FeatureTracker:
	def __init__(self, extract_model, match_model, camera_model, min_cnt=150):
		...
		self.forwframe_ = {
				...
				'descriptor': np.zeros((DESC_SIZE,0)),
				'image': None,
				}
	...
	def readImage(self, new_img):
		...
		if not self.forwframe_['PointID']:
			...
			self.forwframe_['descriptor'] = np.zeros((DESC_SIZE,0))
			# self.forwframe_['descriptor'] = torch.zeros((DESC_SIZE,0))
		...
		print("point extraction time is:", end_time-start_time)
		...
		if self.curframe_['keyPoint'].shape[1] > 0:
			...
			pointID_new = []
			pointID_tracked = []
			descr_new = np.zeros((DESC_SIZE,0))
			descr_tracked = np.zeros((DESC_SIZE,0))
